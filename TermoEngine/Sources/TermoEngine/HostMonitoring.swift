...
/// 内存单位 kB、磁盘 1K 块、显存 MiB。两端共用同一份脚本，避免行为漂移。
public enum HostMonitorScript {
    /// 采样间隔，需与远端 sleep 一致；作为网速 Δt 的兜底（实际优先用 uptime 差更准）。
    public static let interval = 2

    public static let command: String = script.replacingOccurrences(of: "__INTERVAL__", with: String(interval))
...
/// 采样帧解析器：持有上一帧计数器与网速历史，两端共用。
/// 非线程安全——在单一 actor/队列上使用（两端均从主 actor 调用）。
public final class HostMetricsParser {
    /// 一帧的解析结果：metrics=正常帧；unsupported=远端无 /proc（脚本已自行退出，不应重连）。
    public enum Frame {
        case metrics(HostMetrics)
        case unsupported
    }

    public private(set) var netHistory: [NetSample] = []   // 最近若干帧网速，供波动折线图
    public private(set) var netHistoryByInterface: [String: [NetSample]] = [:]
    public private(set) var netTick = 0                    // 每追加一帧自增，驱动折线整条左滑一格
    ...
    /// 新一次采集开始前清空计数器与缓冲：重连后的首帧不产速率（无上一帧可差）。
    public func reset() {
    ...
    public func clear() {
        reset()
        netHistory = []
        netHistoryByInterface = [:]
    }

    /// 以 "===\n" 为帧界逐帧解析；不完整的尾部留在缓冲等下次。
    public func ingest(_ data: Data) -> [Frame] {
        buffer.append(data)
        var frames: [Frame] = []
        let delim = Data("===\n".utf8)
        while let r = buffer.range(of: delim) {
            ...
            if let text = String(data: frame, encoding: .utf8) { frames.append(parse(text)) }
        }
        return frames
    }

    @discardableResult
    public func parse(_ frame: String) -> Frame {
        ...
        let dt = (prevUptime.map { m.uptimeSecs - $0 }).flatMap { $0 > 0 ? $0 : nil } ?? Double(HostMonitorScript.interval)
