...
#ifndef TERMO_SSH_LEGACY_H
#define TERMO_SSH_LEGACY_H
...
const char *termo_ssh_legacy_session_sha256(TermoSSHSession *s);
const char *termo_ssh_legacy_session_md5(TermoSSHSession *s);
...
/// 返回：0=完成、1=超时、2=被取消（termo_ssh_legacy_cancel 置标志）、-1=错误（err 写原因）。
...
/// 阻塞调用线程直到结束；用 termo_ssh_legacy_cancel 从另一线程打断（仅置标志，线程安全）。
