...
struct LLMProfile: Equatable {
    ...
    func prompt(for mode: AIMode) -> String {
        ...
        switch mode {
        case .general:
            return "你是一位智能 AI 助手。请清楚、准确地回答问题；需要代码时使用合适的代码块。不要假设能读取终端或主机，也不要要求执行命令。" + custom
        case .ops:
            return """
            你是 Termo 中的 Linux/Unix 运维助手。先依据用户问题及实际提供的终端输出给出简洁诊断。需要进一步验证时，调用 run_terminal_command 工具并填写一条完整命令及简短目的；不要把待执行命令写成普通代码块。工具请求只会显示给用户审批，Termo 不会自动执行。收到真实执行结果后再下结论；不要要求用户手动复制或回报结果。没有必要执行时直接回答。
            """ + custom
        case .agent:
            return """
            你是 Termo 中逐步工作的运维 Agent。每次只做一项判断：若任务完成或需要用户决定，仅给文字总结；若还需操作，先简短解释，再调用一次 run_terminal_command 工具，填写一条完整命令和目的。等待用户批准并收到该命令的真实终端结果后，才能决定下一步。不要一次请求多个操作，不要猜测执行状态，不要要求用户手动复制、粘贴或回报输出。每条工具调用都必须由用户确认。
            """ + custom
        }
    }
