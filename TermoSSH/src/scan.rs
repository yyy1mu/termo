...
use crate::handler::{handshake_configured, HostPolicy};
use crate::runtime;
...
pub async fn scan_hostkey(
    ...
    let options = crate::options::ConnectionOptions::default();
    scan_hostkey_configured(host, port, real_known_hosts, session_known_hosts, &options).await
}

pub async fn scan_hostkey_configured(
    host: &str,
    port: u16,
    real_known_hosts: &str,
    session_known_hosts: &str,
    ...
    let info = handshake_configured(host, port, Some(policy), options).await?;
...
impl RusshSession {
    ...
    pub async fn exec_stream(
        ...
        if self.is_poisoned() {
            return Err("SSH 操作已取消或连接已断开".into());
        }
        let channel = tokio::select! {
            _ = self.wait_cancelled() => return Ok(()),
            result = tokio::time::timeout(std::time::Duration::from_secs(20),
                self.inner().handle.channel_open_session()) => {
                result.map_err(|_| "通道打开超时".to_string())?
                    .map_err(|e| format!("通道打开失败: {e}"))?
            }
        };
        let request = tokio::select! {
            ...
            result = tokio::time::timeout(std::time::Duration::from_secs(20), channel.exec(true, command)) => result
        };
        if let Err(message) = request
            .map_err(|_| "exec 请求超时".to_string())
            .and_then(|result| result.map_err(|e| format!("exec 失败: {e}")))
        {
            let _ = channel.close().await;
            return Err(message);
        }
        ...
        loop {
            let message = tokio::select! {
                _ = self.wait_cancelled() => {
                    let _ = ch_write.signal(russh::Sig::TERM).await;
                    break;
                },
                message = ch_read.wait() => message,
            };
            match message {
