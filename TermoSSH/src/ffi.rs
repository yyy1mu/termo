...
use crate::connection::ConnectionCancellation;
use crate::forward::{ForwardKind, ForwardSpec, ForwardStateCallback, RusshForward};
use crate::handler::HostPolicy;
use crate::options::{ConnectionOptions, ConnectionOptionsFFI};
use crate::scan::{scan_hostkey_configured, ExecPullCallback};
...
pub unsafe extern "C" fn termo_russh_session_open(
    ...
    fingerprint_cap: i32,
    timeout_ms: i32,
    options: *const ConnectionOptionsFFI,
    err: *mut c_char,
    errlen: i32,
) -> *mut RusshSession {
    let host = read_str(host);
    ...
    let options = match ConnectionOptions::read(options) {
        Ok(mut options) => {
            if timeout_ms > 0 {
                options.timeout = clamp_timeout(timeout_ms);
            }
            options
        }
        Err(message) => {
    ...
    let opened = std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| {
        runtime().block_on(RusshSession::connect_configured(
            &host,
            port as u16,
            &user,
            pass,
...
/// Streaming exec with separate stdout/stderr and a reliable final exit status.
/// # Safety
/// Session and command must be valid. Callback and userdata must remain valid until return.
/// Callback runs synchronously on a runtime worker; no calls occur after this function returns.
#[no_mangle]
pub unsafe extern "C" fn termo_russh_exec_observed(
    s: *mut RusshSession,
    command: *const c_char,
    on_data: Option<unsafe extern "C" fn(*mut std::ffi::c_void, i32, *const c_char, i32)>,
    ...
    if s.is_null() || command.is_null() {
        copy_into(err, errlen, "Invalid exec arguments");
        return -1;
    }
    let address = userdata as usize;
    let observer = on_data.map(|callback| {
        std::sync::Arc::new(move |stderr: bool, data: &[u8]| {
            callback(
        ...
        }) as crate::session::ExecObserver
    });
    match runtime().block_on((&*s).exec_observed(
        &read_str(command),
        Vec::new(),
...
/// Create an independently cancellable operation on the same authenticated transport.
/// # Safety
/// s must be a live session; the returned handle must be closed exactly once.
#[no_mangle]
pub unsafe extern "C" fn termo_russh_session_fork(s: *mut RusshSession) -> *mut RusshSession {
    if s.is_null() {
        return std::ptr::null_mut();
    }
    std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| (*s).fork()))
        .ok()
        .and_then(Result::ok)
...
/// # Safety
/// s must be a live session handle or null.
#[no_mangle]
pub unsafe extern "C" fn termo_russh_session_is_disconnected(s: *mut RusshSession) -> bool {
    s.is_null() || (*s).is_disconnected()
}

/// # Safety
/// s must be a live session handle or null. Disconnects all operations on its transport.
#[no_mangle]
pub unsafe extern "C" fn termo_russh_session_disconnect(s: *mut RusshSession) {
    if !s.is_null() {
        (*s).disconnect();
    }
}
...
/// A cancellation token must outlive every blocking call and concurrent cancel using it.
#[no_mangle]
pub extern "C" fn termo_russh_connection_cancellation_new() -> *mut ConnectionCancellation {
    Box::into_raw(Box::new(ConnectionCancellation::new()))
}

/// # Safety
/// token is valid until all concurrent calls have returned; cancellation is thread-safe.
#[no_mangle]
pub unsafe extern "C" fn termo_russh_connection_cancellation_cancel(
    token: *const ConnectionCancellation,
) {
...
/// # Safety
/// token came from new and no other call may still use it. NULL is accepted.
#[no_mangle]
pub unsafe extern "C" fn termo_russh_connection_cancellation_free(
    token: *mut ConnectionCancellation,
) {
    if !token.is_null() {
...
pub type ConnectionStageCallback =
    unsafe extern "C" fn(*mut std::ffi::c_void, i32, i32, *const c_char);

/// Blocking diagnostic using the real DNS/TCP/handshake/authentication path.
/// # Safety
/// Strings are NUL terminated; token and callback/userdata remain valid until return.
/// No callbacks occur after return. Cancellation never accesses a shared session.
#[no_mangle]
pub unsafe extern "C" fn termo_russh_test(
    host: *const c_char,
    port: i32,
    user: *const c_char,
    password: *const c_char,
    ...
    token: *const ConnectionCancellation,
    options: *const ConnectionOptionsFFI,
    on_stage: Option<ConnectionStageCallback>,
    userdata: *mut std::ffi::c_void,
) {
    let (Some(token), Some(on_stage)) = (token.as_ref(), on_stage) else {
        return;
    };
    let stage = std::cell::Cell::new(1);
    let report = |stage, ok, message: &str| {
        if !token.is_cancelled() {
            let message = std::ffi::CString::new(message.replace('\0', " ")).expect("message");
            on_stage(userdata, stage, ok, message.as_ptr());
        }
    };
    let completed = |value| {
        stage.set(value + 1);
        ...
        report(value, 1, message);
    };
    let result = std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| {
        let host = read_str(host);
        let user = read_str(user);
        if host.is_empty() || user.is_empty() || !(1..=65535).contains(&port) {
            return Some(Err("参数无效（host/user 为空或端口越界）".to_string()));
        }
        let password = read_str(password);
        let key_path = read_str(key_path);
        let key_passphrase = read_str(key_passphrase);
        let policy = Some(HostPolicy {
            host: host.clone(),
            port: port as u16,
            real_known_hosts: read_str(real_known_hosts),
            session_known_hosts: read_str(session_known_hosts),
        });
        let options = match ConnectionOptions::read(options) {
            Ok(v) => v,
            Err(e) => return Some(Err(e)),
        };
        runtime().block_on(token.run(async {
            let (handle, _, _, _, _socket) = crate::handler::connect_and_auth_observed(
                &host,
                port as u16,
                &user,
                (!password.is_empty()).then_some(password.as_str()),
                (!key_path.is_empty()).then_some(key_path.as_str()),
                (!key_passphrase.is_empty()).then_some(key_passphrase.as_str()),
                &options,
                policy,
                &completed,
            )
            .await?;
            // The socket guard closes this diagnostic even if disconnect is stalled/cancelled.
            let _ = tokio::time::timeout(
                std::time::Duration::from_secs(1),
                handle.disconnect(russh::Disconnect::ByApplication, "test complete", "en"),
            )
            .await;
            Ok(())
        }))
    ...
    match result {
        Ok(Some(Ok(()))) => completed(5),
        Ok(Some(Err(message))) => report(stage.get(), 0, &message),
        Ok(None) => {}
        Err(_) => report(stage.get(), 0, "内部 panic（已被 FFI 边界拦截）"),
    }
}
...
pub unsafe extern "C" fn termo_russh_scan_hostkey(
    ...
    if host.is_null() || out.is_null() || !(1..=65535).contains(&port) {
        return -1;
    }
    let host = read_str(host);
    let real = read_str(real_known_hosts);
    ...
    let result = std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| {
        runtime().block_on(async {
            ...
            let scan = scan_hostkey_configured(&host, port as u16, &real, &session, &options);
            if let Some(token) = token.as_ref() {
                token.run(scan).await
            } else {
                Some(scan.await)
            }
        })
...
/// 重命名；overwrite=1 → posix-rename 原子覆盖；失败返回原始状态，不删除目标或重试。
///
/// # Safety
