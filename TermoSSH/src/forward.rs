    ...
    pub async fn forward_open(
        ...
        match kind {
            ...
            ForwardKind::Remote => {
                ...
                let route = (bind.to_string(), port);
                let (slot_tx, mut slot_rx) = mpsc::unbounded_channel();
                {
                    let mut routes = self.inner().forward_slot.lock().expect("forward routes");
                    if routes.contains_key(&route) {
                        return Err("远程监听已存在".into());
                    }
                    routes.insert(route.clone(), slot_tx);
                }
                if let Err(error) = self.inner().handle.tcpip_forward(bind.as_ref(), port).await {
                    self.inner()
                        .forward_slot
                        .lock()
                        .expect("forward routes")
                        .remove(&route);
                    return Err(format!("转发请求被拒绝: {error}"));
                }
                let inner = Arc::clone(self.inner());
                ...
                let join = runtime().spawn(async move {
                        ...
                        tokio::select! {
                            channel = slot_rx.recv() => match channel {
                                Some(channel) => {
                                    ...
                                    let semaphore = Arc::clone(&semaphore);
                                    connections.spawn(async move {
                                        // 提前转为自动关闭的 stream，连接目标失败/取消也释放通道。
                                        let mut stream = channel.into_stream();
                                        let Ok(_permit) = semaphore.try_acquire_owned() else { return };
                    ...
                    inner
                        ...
                        .remove(&route);
                    slot_rx.close();
                    while let Ok(channel) = slot_rx.try_recv() { drop(channel.into_stream()); }
                    connections.shutdown().await;
                    let _ = tokio::time::timeout(CHANNEL_OPEN_TIMEOUT,
                        inner.handle.cancel_tcpip_forward(&bind_owned, port)).await;
                });
                Ok(RusshForward {
                    close_tx: Mutex::new(Some(close_tx)),
                    join: Mutex::new(Some(join)),
