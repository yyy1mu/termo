...
/// 每个监听地址/端口独立路由，同一 SSH 传输支持多个 -R 规则。
pub(crate) type ForwardSlot = Mutex<
    HashMap<(String, u32), tokio::sync::mpsc::UnboundedSender<russh::Channel<russh::client::Msg>>>,
>;

fn route_forward<T: Clone>(
    routes: &HashMap<(String, u32), T>,
    address: &str,
    port: u32,
) -> Option<T> {
    if let Some(exact) = routes.get(&(address.to_owned(), port)) {
        return Some(exact.clone());
    }
    // sshd 可能把 wildcard/localhost 规范化；只在端口唯一时允许回退，绝不猜测多个目标。
...
impl client::Handler for ProbeHandler {
    ...
    async fn server_channel_open_forwarded_tcpip(
        ...
        let sender = route_forward(
            &self.forward_slot.lock().expect("forward slot"),
            connected_address,
            connected_port,
        );
        if sender.as_ref().is_some_and(|s| !s.is_closed()) {
            reply.accept().await;
            if let Err(error) = sender.expect("checked above").send(channel) {
                drop(error.0.into_stream());
            }
        }
        Ok(())
    }
...
pub(crate) async fn connect_and_auth_configured(
    host: &str,
    port: u16,
    user: &str,
    password: Option<&str>,
    ...
    String,
> {
    let (handle, fingerprint, forward_slot, host_info, socket) = connect_and_auth_observed(
        host,
        port,
        user,
        password,
...
/// Reports completed stages on the actual connection; the caller owns socket cleanup
/// until it either hands the transport to a session or finishes its diagnostic.
#[allow(clippy::too_many_arguments)]
pub(crate) async fn connect_and_auth_observed(
    host: &str,
    port: u16,
    user: &str,
    password: Option<&str>,
    ...
    (
        ...
        Arc<ForwardSlot>,
        Arc<HostKeyInfo>,
        crate::connection::ConnectionSocket,
    ),
    String,
> {
    let work = async {
        ...
        let handler = ProbeHandler {
            ...
            forward_slot: Arc::new(Mutex::new(HashMap::new())),
        ...
        let (stream, socket) = crate::connection::ConnectionSocket::connect(
            host,
            port,
            completed,
        ...
        let mut handle: Handle<ProbeHandler> = client::connect_stream(config, stream, handler)
            .await
            .map_err(|e| {
        ...
        completed(4);
        Ok((handle, fingerprint, forward_slot, host_info, socket))
    };
    tokio::time::timeout(options.timeout, work)
        .await
        .map_err(|_| "连接/认证超时".to_string())?
}
...
pub(crate) async fn handshake_only(
    ...
    handshake_configured(host, port, policy, &options).await
}

pub(crate) async fn handshake_configured(
    host: &str,
    port: u16,
    policy: Option<HostPolicy>,
    options: &crate::options::ConnectionOptions,
    ...
    let host_info = handler.host_info.clone();
    let work = async {
        let (stream, _socket) = crate::connection::ConnectionSocket::connect(
            host,
            port,
            &|_| {},
            options.proxy.as_ref(),
        ...
        .await?;
        let handle: Handle<ProbeHandler> = client::connect_stream(config, stream, handler)
            .await
            .map_err(|e| format!("握手失败: {e}"))?;
        let _ = handle
            .disconnect(russh::Disconnect::ByApplication, "scan", "en")
            .await;
        Ok(host_info)
    };
    tokio::time::timeout(options.timeout, work)
        .await
        .map_err(|_| "主机指纹扫描超时".to_string())?
}
...
mod trust_tests {
    ...
    #[test]
    fn forward_routes_do_not_cross_ports_or_ambiguous_bind_addresses() {
        let mut routes = HashMap::from([
            (("localhost".to_owned(), 8080), 1),
            (("0.0.0.0".to_owned(), 9090), 2),
        ]);
        assert_eq!(route_forward(&routes, "127.0.0.1", 8080), Some(1));
        assert_eq!(route_forward(&routes, "192.0.2.1", 9090), Some(2));
        assert_eq!(route_forward(&routes, "127.0.0.1", 7070), None);
        routes.insert(("127.0.0.2".to_owned(), 8080), 3);
        assert_eq!(route_forward(&routes, "127.0.0.1", 8080), None);
        assert_eq!(route_forward(&routes, "127.0.0.2", 8080), Some(3));
        routes.remove(&("0.0.0.0".to_owned(), 9090));
        assert_eq!(route_forward(&routes, "localhost", 8080), Some(1));
    }
