...
当前 macOS 是完整产品；iOS 和 Apple Watch 已有独立的 App 入口、导航、共享模型与基础数据流，但尚未接入 SSH 连接。不要直接把 `Mac/` 的 AppKit 视图或 macOS `AppModel` 加进移动端 target。

| 目录 | 职责 |
| --- | --- |
| `Mac/` | macOS App、Rust SSH 桥接、终端、SFTP 与现有同步实现 |
| `TermoCore/Sources/TermoCore/` | 纯 Foundation 的跨平台数据契约，不依赖 AppKit、UIKit 或 WatchConnectivity |
| `iOS/App/` | iPhone/iPad 生命周期和根导航 |
| `iOS/Features/` | iOS 页面；当前有主机资料编辑和设置说明 |
| `iOS/Services/` | iOS 本地存储与发给 Watch 的资料投影 |
| `Watch/App/` | watchOS 生命周期 |
| `Watch/Features/` | Watch 页面；当前显示 iPhone 送来的主机名称 |
| `Watch/Services/` | WatchConnectivity 接收与最后一次快照缓存 |
