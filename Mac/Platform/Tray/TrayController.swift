...
final class TrayController: NSObject, NSMenuDelegate {
    ...
    func menuNeedsUpdate(_ menu: NSMenu) {
        ...
        let show = NSMenuItem(title: String(localized: "显示 Termo", bundle: AppSettings.localizationBundle, locale: AppSettings.activeLocale), action: #selector(showTapped), keyEquivalent: "")
        show.target = self
        menu.addItem(show)
        menu.addItem(.separator())

        let tasks = AppModel.shared.runningBackgroundSummaries
        let header = NSMenuItem(title: tasks.isEmpty ? String(localized: "无后台任务", bundle: AppSettings.localizationBundle, locale: AppSettings.activeLocale) : String(localized: "后台任务（\(tasks.count)）", bundle: AppSettings.localizationBundle, locale: AppSettings.activeLocale),
        ...
        if tasks.count > maxRows {
            let more = NSMenuItem(title: String(localized: "  还有 \(tasks.count - maxRows) 项，打开应用查看", bundle: AppSettings.localizationBundle, locale: AppSettings.activeLocale),
        ...
        menu.addItem(.separator())
        let quit = NSMenuItem(title: String(localized: "退出 Termo", bundle: AppSettings.localizationBundle, locale: AppSettings.activeLocale), action: #selector(quitTapped), keyEquivalent: "")
        quit.target = self
        menu.addItem(quit)
    }
